...
| `gpn7eWFl` | `scripts/spike/step.sh` + `src/core/workspace.ts` | **Harness regression.** `step.sh` passes `--env KB_SESSION_ID=<scenario id>` to the Pi container (for file-backed `SessionState`); inside the container `FileSystemWorkspaceStorage.getActiveWorkspaceId()` then reads `/tmp/.mykb-session-<id>` (ephemeral, empty — nothing wrote it) → `null` → `before_agent_start` injects no `<mykb-workspace>` and never calls `state.setBoostedAreas()`. The harness writes `.active`; the container reads the session file; they never meet. Effect: `journal-auto-inject` + `handoff` matrices are RED; blocks workspace-dependent scenarios (`kb-work-tools`, area-scoring `workspace-boost`). Pre-seed-based scenarios (`scoring-isolated`, `file-path-signal`) are unaffected. | Ad-hoc kb-spike probe 2026-05-11 ("I can see mykb-areas. I cannot see mykb-workspace.") | 🐛 Documented; tracked in [issue #5](https://github.com/vilosource/mykb/issues/5) (fix options there — touches `docs/session-isolation-DESIGN.md`) |
