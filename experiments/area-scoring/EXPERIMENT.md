...
| An area loaded in turn N gets a sticky-boost in turn N+1; equally-scoring competitor doesn't | LLM cites the sticky area's fact in turn N+1 over a freshly-matched competitor | `sticky-area-persistence` | 🚧 blocked — see [issue #6](https://github.com/vilosource/mykb/issues/6) |
| Many areas score; total entries exceed the 2000-token budget | Highest-scoring areas keep their entries; lower-scoring areas drop entries first; widget marker stays in (because widget area scored highest) | `token-budget-eviction` | 🚧 blocked — see [issue #6](https://github.com/vilosource/mykb/issues/6) |
...
- **`sticky-area-persistence`**: two-step (or single-step with `loadedAreas` pre-seeded). Step 1 establishes signals matching area X (loads it → marked sticky). Step 2 asks a prompt that scores X and a fresh competitor Y equally on keywords, with Y's first entry large enough to fill the token budget; assert X wins because of the sticky boost (relies on `selectEntriesForInjection`'s deterministic area-id tie-break for the RED case). **Blocked** — see [issue #6](https://github.com/vilosource/mykb/issues/6): a first attempt had specimen areas getting injected from a nonce-only signal; root cause not yet found.
- **`token-budget-eviction`**: prepare 5+ synthetic areas with distinct keyword-overlap signals; the marker fact lives only in the highest-scoring area. Total entry text exceeds 2000 tokens. Assert the marker reaches the LLM (top area kept) AND a low-scoring area's marker does NOT (evicted). **Blocked on the same investigation** ([issue #6](https://github.com/vilosource/mykb/issues/6)).
